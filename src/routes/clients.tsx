...
function ClientsPage(){
  ...
  return <div className="mx-auto max-w-6xl space-y-8"><input ref={inputRef} hidden type="file" accept=".xlsx,.csv,.tsv,.docx" onChange={e=>upload(e.target.files?.[0])}/><PageHeader eyebrow="מקור נתונים" title="נתוני לקוחות" description="בחר קובץ Excel, CSV או Word. השורה הראשונה משמשת ככותרות והמערכת ממפה אותן למשתני התהליך." actions={<Button onClick={choose}><Upload className="size-4"/>טעינת קובץ</Button>}/>
